...
class CarliniWagnerL2_WB_TF(object):
    ...
    def __init__(self, sess, model, ensemble, batch_size, confidence, targeted,
        ...
        n_nets = np.array([len(x) for x in self.ensemble])
        n_nets_max = np.max(n_nets)
        ...
        if self.TARGETED:
            ...
            # self.target = tf.tile(ZERO_ONE, [batch_size, n_nets_max, 1])
            # print(self.target)
            diff = label_nets[:, :, 0] - label_nets[:, :, 1]
            print(diff)
        else:
            # DOUBLE-CHECK
            pass
