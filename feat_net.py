...
from keras.layers import (Activation, Dense, Dropout, Flatten, Lambda, Conv2D,
                          MaxPooling2D, Reshape, Concatenate, Cropping2D)
...
class FeatNet():
    ...
    def __init__(self, input_shape, output_shape, crop_pos, learning_rate=1e-3,
                 stn_weight=None, save_path="model/featnet.ckpt"):
        ...
        rescale1 = Lambda(lambda x: x*2 - 1., 
                         input_shape=(32, 32, 3), 
                         output_shape=(32, 32, 3))(self.x)
        stn = SpatialTransformer(localization_net=locnet_v3(), 
                                 output_size=(32, 32),
                                 trainable=False,
                                 weights=stn_weight)(rescale1)
        rescale2 = Lambda(lambda x: x*.5 + .5, 
                          input_shape=(32, 32, 3), 
                          output_shape=(32, 32, 3))(stn)
        
        # Build model
        with tf.variable_scope("featnet"):
            ...
            for pos in crop_pos:
                ...
                crop = Cropping2D(cropping=((top, height - bot), 
                                            (left, width - right)))(rescale2)
            ...
            concat = Concatenate(axis=-1)(self.feat_scores)
            ...
            SCORE_THRES = 1.
            self.output = tf.reduce_sum(concat, axis=1, keepdims=True)
            scaled_y = 2.*self.y - 1.
            pred = tf.maximum(0., self.n_feats*SCORE_THRES - self.output)
            self.loss = tf.reduce_mean(tf.multiply(scaled_y, pred))

        # Set up optimizer
        with tf.variable_scope("featnet_opt"):
    ...
    def predict_model(self, sess, x, batch_size=128):
    ...
    def eval_model(self, sess, data, batch_size=128):
        ...
        x, y = data
        output = self.predict_model(sess, x, batch_size=batch_size)

        # TODO: use self.loss instead
        y_thres = output >= self.n_feats * 0.75
        accuracy = np.mean(np.equal(y_thres, y))
        loss = np.mean(np.multiply(2.*y - 1., self.n_feats - output))
